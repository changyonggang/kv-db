# kv-db
inspired by etcd-raft-example and leveldb, etcd's [raft library](../../raft). It provides a simple REST API for a key-value store cluster backed by the [Raft][raft] consensus algorithm.
...
```sh
go build -o kv-db cmd/main.go
```

### Running single node kv-db

First start a single-member cluster of kv-db:

```sh
mkdir data
kv-db --id 1 --cluster http://127.0.0.1:12379 --port 12380
```

Each kv-db process maintains a single raft instance and a key-value server.
...
This will bring up three kv-db instances.
...
```sh
kv-db --id 1 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 12380
kv-db --id 2 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 22380
kv-db --id 3 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379 --port 32380
```
...
```sh
kv-db --id 4 --cluster http://127.0.0.1:12379,http://127.0.0.1:22379,http://127.0.0.1:32379,http://127.0.0.1:42379 --port 42380 --join
```
...
The kv-db consists of three components: a raft-backed key-value store, a REST API server, and a raft consensus server based on etcd's raft implementation.
...
For kv-db, this commit channel is consumed by the key-value store.
